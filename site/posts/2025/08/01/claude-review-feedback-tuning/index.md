...
[前回の記事](/posts/2025/07/02/claude-code-action-pr-review/)でClaude Code Actionを使ったPRレビューの自動化について紹介した。
...
1. フィードバック収集: 週2回の定期実行で、過去のPRから開発者の反応を拾う
...
スクリプトは最近のマージ済みPRから、Claudeのレビューコメントとそれに対する開発者の反応を拾ってくる。
特に見ているのは以下のパターン。
...
こうしておくと、レビューの形は揃えたまま、プロジェクト固有のコンテキストだけを動かせる。
フィードバックループで書き換わるのは `custom_instructions` だけなので、レビューの骨格は動かないままで済む。
...
余談だが、Copilotのレビューにはこちらの狙いから外れた指摘も入ってくる。それにClaude Codeが引きずられる場面もあったので、Copilotの自動レビューはオフにしている。
...
収集スクリプトでは `claude[bot]` を見つけ、GitHub API経由で直近のマージ済みPRからまとめて取ってくる。
...
週2回の自動実行で、レビューの質は少しずつ寄っていく。回しすぎない間隔にしておくと、ガイドラインが落ち着いたまま、反応も取りこぼさずに済む。

プロジェクト固有の文脈をどこまで書くかは悩ましい。細かく書くほど手入れが重くなるので、大きな設計判断だけに絞っている。
...
AIツールは入れて終わりではなく、使う人の反応を取り込みながら直し続けないと形にならない、と実感した。
...
定期実行のたびに実際の反応から学び、以前より役に立つレビューが返ってくるようになった。

より短いフィードバックループの実現や、プロジェクト間でのナレッジ共有も今後の検討課題だ。
