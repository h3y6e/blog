...
[前回の記事](/posts/2025/08/01/claude-review-feedback-tuning/)では、Claudeのレビューに対する開発者の反応を収集し、`.github/copilot-instructions.md` と `CLAUDE.md` を自動更新する仕組みを紹介した。

この仕組み自体は、いまは所属チームの複数プロダクトでかなり広く使っている。
指摘の妥当性やコンテキストの読み方という点で、既存の自動レビューとは手応えが違う。実際、リポジトリ次第では人間がほとんどレビューしておらず、Claudeとのやり取りを中心とした運用になっている。
...
`code-review-guidelines` には自動更新用のマーカーを置いた。
...
現在の `claude-review` は、自動レビュー用と対話用の2系統を1つのreusable workflowにまとめている。
...
レビューの再確認や説明依頼であればcomment-onlyのまま応答し、明示的に修正を依頼されたときだけコード変更に入るように分けている。
...
と明示している。
これで、レビュー基準とリポジトリ固有知識を分けて保てるようになった。

ここで、promptやrepository contextを自動的に増やさないのは意図的である。
...
自分たちの運用で見えていたこととも、この結論はよく一致している。以前はレビュー基準とプロジェクト知識をまとめて増やしていくと、重要ではない指摘や、チームとして意図している設計へのコメントが紛れ込みやすくなった。

だからこそ現在のtuningでは、何でもかんでも知識を足すのではなく、繰り返し役に立ったルールを `code-review-guidelines` へ寄せている。
リポジトリ固有で安定した事実は `project-knowledge` にまとめた。
前回のようにprompt本体を自動改善し続けるより、最小限だけをskillとして保つ方が安全だと考えている。

## 現在の構成の良いところ

まず、何を調整しているのかが前よりはっきり見える。
...
今後もリポジトリごとの差を吸収しつつ、最小限のcontextでどこまでレビュー品質を維持できるかを引き続き見ていけたらと思う。
