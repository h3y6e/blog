...
アプリストアのレポートを取り込むプロジェクトで、まず何が取得できるのかを調べる必要があった。ストアごとに取得の方法が何通りもあり、方法ごとに保持期間、必要な権限、制約が違う。
...
許可の出たミラーはコミットする。コミットしてあれば、ページの記述を取得元のデータと後から確かめられるからだ。許可の出ないものは `.gitignore` で除く。
...
コードのあるべき姿を述べる点で、ページはspecと重なる。ただしその記述は工程に属さず、出典と一致しているかどうかだけで正しさが決まる。
