...
Webサービスや社内ツールなど、エンドユーザーがバージョン番号を見て依存関係を判断するわけではないものがこれにあたる。こうしたプロダクトでは何が公開APIなのかが曖昧で、majorかminorかpatchかの判断に意味のないコストがかかりやすい。
...
- いつリリースされたかが一目でわかる: アプリケーションでは「そのバージョンがいつ出たか」が重要なことが多く、CalVerならバージョン番号自体がそれを伝える

OSではUbuntu (`YY.0M.MICRO`) やiOS, macOS (`YY.MINOR`)、iOSアプリではBitwarden (`YYYY.MM.MICRO`) やMastodon (`YYYY.MINOR`)、開発ツールではpip (`YY.MINOR.MICRO`) やUnity (`YYYY.MINOR.MICRO`) など、幅広く採用されている。
...
SemVerモードのtagprでは、マージされたPRに `major` / `minor` ラベルが付いていればそれに応じたバージョンbumpを行い、なければpatchのbumpになる。リリースPRに `tagpr:major` や `tagpr:minor` ラベルを手動で付けて調整することもできる。SemVerで使うとしても素直に使える。
